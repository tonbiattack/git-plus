...
//  - Branch: コミットが所属するブランチ名
//  - Author: コミット作成者名
//  - Hours: 推定作業時間（時間単位、小数点第1位まで）
//  - Timestamp: コミット日時
//  - Files: 変更されたファイルのリスト
//
// 使用箇所:
//  - displayCommitStats: コミットごとの統計を表示
...
// main はコミット履歴から作業時間を自動集計するメイン処理
//
// 処理フロー:
//  1. コマンドライン引数を解析（期間、スコープ、表示件数など）
//  2. 期間の計算（-w/-m/-y オプションから --since を計算）
//  3. 指定期間のコミット履歴を取得
//  4. ブランチと作成者でコミットをグループ化
//  5. 各コミットの作業時間を推定（次のコミットとの時間差）
//  6. コミット統計を新しい順に表示
//  7. 結果をファイルに保存
//
// 使用するgitコマンド:
//  - git log --all/--remotes/--branches --format=%H|%D|%an|%at|%s: コミット履歴を取得
//  - git diff-tree --no-commit-id --name-only -r <hash>: コミットの変更ファイルを取得
//  - git branch --contains <hash> --format=%(refname:short): コミットを含むブランチを取得
//
// 実装の詳細:
//  - 作業時間の推定ルール:
//    - 同じブランチ・同じ作成者の次のコミットとの時間差が2時間以内: その時間を作業時間とする
//    - 2時間を超える場合または最後のコミット: デフォルトで30分（0.5時間）
//  - stash と detached HEAD は除外
//  - デフォルトスコープ: all（全ブランチ）
//  - デフォルト表示件数: 100件（--limit で変更可能）
//
// 終了コード:
//  - 0: 正常終了
