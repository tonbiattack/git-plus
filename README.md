...
3. **同じブランチ・同じ作成者の連続コミット間が2時間以内の場合、その時間を作業時間として計算します。**
...
- `--limit, -l <数>`: コミット別表示時の表示件数（デフォルト: 20、0で全件表示）
- `--help, -h`: ヘルプを表示
...
- **カスタマイズ可能な表示件数**: コミット別表示時に`--limit`で表示件数を調整可能。大量のコミットがある場合でも見やすく表示できます。
- **貢献者分析**: ブランチごとに各作成者のコミット数と割合を表示し、チーム内の作業分担を可視化できます。

**注意事項:**
- `--scope remotes`（デフォルト）の場合、事前に`git fetch`を実行して最新のリモート情報を取得してください。
