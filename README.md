...
- `git step`：リポジトリ全体のステップ数とユーザーごとの貢献度を集計します。現在のコードに占める割合を正確に表示します。
...
3. コード割合が多い順に表示されます。
4. 各作成者の追加行数、削除行数、更新行数、現在のコードベースに残っている行数、コード割合を表示します。
...
- **複数の指標を表示**:
  - **追加行数**: 過去に追加した行数の累計（削除された行も含む）
  - **削除行数**: 過去に削除した行数の累計
  - **更新行数**: 追加と削除の合計。変更に関わった総行数を示す
  - **現在行数**: `git blame`で取得した、現在のコードベースに残っている行数
  - **コード割合**: 現在のリポジトリ総行数に占める割合（全ユーザーの合計が100%になる）
- **期間指定**: 特定の期間に限定した集計が可能です。
- **初回コミット除外**: デフォルトで初回コミットを除外することで、より実態に即した統計が得られます。

**注意事項:**
- バイナリファイルの行数は集計から除外されます。
- **現在行数**は`git blame`を使用して、現在のコードベースに実際に残っている各ユーザーの行数を正確に集計します。
- **コード割合**は、現在のリポジトリ総行数に対する各ユーザーの現在行数の割合です（合計100%）。
- **更新行数**が大きいほど、そのユーザーがコードに多くの変更を加えたことを示します。
- リファクタリングなどで他のユーザーが行を上書きした場合、元の作成者の現在行数は減少します。
