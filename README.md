...
2. 同名のローカルブランチが存在する場合は、「再作成しますか？ (y/n):」というプロンプトが表示されます：
   - `y` / `yes` / 空入力: ブランチを削除して作り直します
   - `n` / `no`: 既存のブランチに切り替えます
   - その他の入力: 処理を中止します
3. 再作成を選択すると、既存ブランチを強制削除してから新しいブランチを作成します。
4. 切り替えを選択すると、既存ブランチに `git checkout` で切り替えます。
